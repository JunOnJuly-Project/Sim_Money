...
작업 도중 발견된 신규/추천 작업, 또는 HANDOFF/계획서에 명시된 미완료 슬라이스가 있다면
**사용자 확인 없이 권장 우선순위대로 자동으로 연속 진행**한다. 글로벌 룰(L3 엄격도, 커밋-문서
동기화, CLI 자동 허가 범위 등) 은 그대로 준수한다. 실패·블로커·범위 변경 시에만 보고/질문.
